...
$('.oe_website_sale').each(function () {
    ...
    $(oe_website_sale).on('click', 'a.js_check_product1', function(){
        var user_bet = document.getElementsByName('get_bet')[0].value;
        num_user_bet = parseFloat(user_bet);

        var $ul = $(this).parents('ul.class_bet:first');
        var $parent = $ul.closest('.js_product');
        var $bet_form = $parent.find('div.show_hide_bet').first();
        if(num_user_bet) {
            openerp.jsonRpc("/bets", 'call', {
                'user_bet': user_bet
            }).then(function (result) {
                $('.show_hide_bet').hide("slow");
                $('.container').show(1000);
            })
        }
    })
