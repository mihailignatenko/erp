...
class website_sale(http.Controller):
    ...
    ############
    def set_status_end_auction(self, cr, pool, context, id_auction=0):
            auction_ids = pool.get('auction.edit').search(cr, SUPERUSER_ID, [])
            auctions_ids = lambda id: pool.get('auction.edit').browse(cr, SUPERUSER_ID, id, context=context)
            bets_ids = pool.get('like.bits').search(cr, SUPERUSER_ID, [])
            bets_ids_browse = lambda id: pool.get('like.bits').browse(cr, SUPERUSER_ID, id, context=context)
            finish_search = pool.get('finish.auctions').search(cr, SUPERUSER_ID, [])
            finish_browse = lambda id: pool.get('finish.auctions').browse(cr, SUPERUSER_ID, id, context=context)
            date_stop = auctions_ids(id_auction).stop_auction_date_time
            auctions = pool.get('auction.edit')

            date_object_stop = datetime.strptime(date_stop, '%Y-%m-%d').date()
            status = False

            list_ = []
            ids_finish_aucion = []

            if date_object_stop == datetime.today().date():
                try:
                    #write false in field is_active
                    auctions.write(cr, SUPERUSER_ID, [id_auction], {'is_active': status})
                    #form request to create win user
                    for id_bet in bets_ids:
                        print id_bet, "BET -------"
                        if auctions_ids(id_auction).products.product_variant_ids.id == bets_ids_browse(id_bet).product_id:
                            user_bet = {'user_win': bets_ids_browse(id_bet).user_id,
                                        'user_bet': bets_ids_browse(id_bet).u_bit,
                                        'product': bets_ids_browse(id_bet).product}
                            list_.append(user_bet)
                    win = max(list_, key=lambda x: x['user_bet'])
                    win['auction_finish'] = id_auction
                    if finish_search:
                        print finish_search
                        for finish_id in finish_search:
                            ids_finish_aucion.append(finish_browse(finish_id).auction_finish.id)
                        print ids_finish_aucion
                        if id_auction not in ids_finish_aucion:
                            finish_obj = request.registry.get('finish.auctions')
                            finish_obj.create(cr, SUPERUSER_ID, win, context=context)
                    else:
                        finish_obj = request.registry.get('finish.auctions')
                        finish_obj.create(cr, SUPERUSER_ID, win, context=context)
...
############
        try:
            ...
            website_sale.current_product_id = product_id

            auction_ids = pool.get('auction.edit').search(cr, SUPERUSER_ID, [])
            product_ids = lambda id: pool.get('auction.edit').browse(cr, SUPERUSER_ID, id, context=context)
            minimum_bet = lambda id_auc: product_ids(id_auc).minimum_bid
            status_auction = lambda id: product_ids(id).is_active
            pr = set()
            for id in auction_ids:
                if product_id == product_ids(id).products.product_variant_ids.id:
                    pr.add(id)
                    self.set_status_end_auction(cr, pool, context, id)
            for _id in pr:
                # print "Auction name", product_ids(_id).auction_name, "bet", minimum_bet(_id)
                default_bet = minimum_bet(_id)
                if status_auction(_id):
                    context.update(default_bet=default_bet)
        except:
            print "Could not get a bet"
############
...
class AuctionRoute(http.Controller):
    """Auction route class"""

    @http.route(['/bets'], type='json', auth='user', website=True)
    def route_bet(self, user_bet=0):
        cr, uid, context, pool = request.cr, request.uid, request.context, request.registry
        current_user_id = uid
        ####
        auctions_ids = pool.get('auction.edit').search(cr, uid, [])
        product_ids = pool.get('product.product').search(cr, uid, [])
        auction_id = lambda id: pool.get('auction.edit').browse(cr, uid, id, context=context)
        product = lambda id: pool.get('product.product').browse(cr, uid, id, context=context)
        user_login = pool.get('res.users').browse(cr, uid, current_user_id, []).login
        product_id = website_sale.current_product_id
        if product_id == 0:
        ...
        val = {
            ...
            'date_bit': str(datetime.now()),
            'product': product_name,
            'product_id': product_id
        }
        try:
            bet_obj = request.registry.get('like.bits')
            bet_id = bet_obj.create(cr, uid, val, context=context)

            print "created ", val, "\n"
            if bet_id:
                self.write_min_bet_with_step(cr, pool, product_id, user_bet)

        except (Exception) as e:
            print "Error create tracker"
            traceback.print_exc()
            for v in val:
                pass
                print v, val[v]

    def write_min_bet_with_step(self, cr, pool, product_id=None, user_bet=0):
        auction_ids = pool.get('auction.edit').search(cr, SUPERUSER_ID, [])
        products_ids = lambda id: pool.get('auction.edit').browse(cr, SUPERUSER_ID, id).products.product_variant_ids.id
        auctions_ids = lambda id: pool.get('auction.edit').browse(cr, SUPERUSER_ID, id).step
        auction_write = lambda id_auction, value=0: pool.get('auction.edit').write(cr, SUPERUSER_ID, [id_auction], {'minimum_bid':value})
        for auction in auction_ids:
            if product_id == products_ids(auction):
                step = auctions_ids(auction)
                value = float(user_bet)+step
                auction_write(auction, value)
                print "Write"
